...
static const char
    ...
    *fpe_plain = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTU",
...
static void check(const char* method, void* result, void* expected, size_t size)
{
...
int main()
{
    ...
    *output ^= AES_EAX_decrypt(key, iv, test, st, a, sa, 16, output);
    ...
    memcpy(input, "890121234567890000", st);
    memcpy(test, "750918814058654607", st);
#if CUSTOM_ALPHABET == 0
    st = 18;
    AES_FPE_encrypt(key, a, input, st, output);
    check("FF1 encryption", output, test, st);
    AES_FPE_decrypt(key, a, test, st, output);
    check("FF1 decryption", output, input, st);
#endif
