...
#define BuffL               ((TestStringSize / 2 + 31) & ~0xF)
...
#if AES___    == 192                      /*  ↓↓↓↓  PKCS#7 is enabled */
...
#endif
    *cmac_ptxt = "b887df1fd8c239c3 e8a64d9822e21128",
...
{
    uint8_t shl = 4;
    for (*bytes = 0; *str; shl = 4 * !shl)
    {
        while (*str < '0' || *str > 'f')  ++str;
        *bytes |= (*str % 16 + (*str > '9') * 9) << shl;
        if (*++str && shl == 0)  *++bytes = 0;
    }
}

int main()
{
    uint8_t iv[16], key[64], authKey[32], input[BuffL - 16], test[BuffL], output[BuffL],
           *a = authKey + 1, sa = sizeof authKey - 1, st = TestStringSize / 2;
    str2bytes(cipherKey, key);
    str2bytes(secondKey, key + 32);
    str2bytes(secretKey, authKey);
    str2bytes(iVec, iv);
    str2bytes(plainText, input);
#if M_RIJNDAEL
    str2bytes(iVec, input + 48);
    str2bytes(secondKey, test);
    a = AES_KEY_LENGTH == 16 ? key : input + (AES___ - 192) / 2;
    ...
    AES_GCM_decrypt(key, iv, test, st, a, sa, 16, output);
    ...
    *output ^= GCM_SIV_decrypt(key, iv, test, st, a, sa, 16, output);
    ...
    AES_EAX_decrypt(key, iv, test, st, a, sa, 16, output);
#endif
    check("EAX decryption", output, input, st);
#endif
#if FPE && AES_KEY_LENGTH + CUSTOM_ALPHABET == 16
    ...
    AES_KEY_wrap(authKey, key + 32, AES_KEY_LENGTH, output);
    check("key wrapping  ", output, test, AES_KEY_LENGTH + 8);
    AES_KEY_unwrap(authKey, test, AES_KEY_LENGTH + 8, output);
    check("key unwrapping", output, key + 32, AES_KEY_LENGTH);
#endif
    /** a template for "OFFICIAL TEST VECTORS":  */
#if OCB * EAX * SIV * GCM_SIV * POLY1305 * FPE * (16 / AES_KEY_LENGTH)
    ...
    *output ^= AES_SIV_decrypt(key, test, test + 16, st, a, sa, output);
    ...
    str2bytes("01020304050607080102030405060708", key);
    str2bytes("A20D060B607C86F7540116007BC175A8\
               03020100BE0D280B810984A60C060A60\
               7C86F7540116007B040248F3C2040330\
               0005", input);
    str2bytes("515AE775", test);
    AES_EAX_encrypt(key, input, NULL, st, sa, output);
    check("EAX encryption", output, test, st + 4);
    st += AES_EAX_decrypt(key, input, test, st, sa, output);
    check("EAX decryption", output, input, st);
    st = 28; sa = 65;   /* from Moise-Beroset-Phinney-Burns paper: */
    str2bytes("10 20 30 40 50 60 70 80 90 a0 b0 c0 d0 e0 f0 00", authKey);
    ...
    AES_EAX_encrypt(authKey, test, input, st, sa, output);
    check("EAX encryption", output, key, st + 4);
    AES_EAX_decrypt(authKey, test, key, st, sa, output);
#else
