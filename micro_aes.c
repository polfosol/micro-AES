...
static uint8_t constmemcmp( const void* src, const void* dst, const uint8_t n )
{
    const volatile char *p1 = src, *p2 = (const volatile char*) dst;
                      ...
                      const void* pntxt, const size_t ptextLen, void* crtxt )
{
    ...
    xorBlock( G, iv );
    memcpy( mac, iv, GCM_TAG_LEN );              /*  GMAC = Enc(iv) ^ G-HASH  */
}
...
{
    uint8_t* mac = (uint8_t*) crtxt + ptextLen;
    block_t iv = { 14 - CCM_NONCE_LEN, 0 }, T;

    memcpy( iv + 1, nonce, CCM_NONCE_LEN );
    AES_setkey( key );
    CCMtag( iv, aData, pntxt, aDataLen, ptextLen, T );
    CTR_cipher( iv, CCM_GCM, pntxt, ptextLen, crtxt );
    BURN( RoundKey );

    memcpy( mac, T, CCM_TAG_LEN );
}
                      ...
                      const void* crtxt, const size_t crtxtLen, void* pntxt )
{
    ...
    AES_setkey( key );
    CTR_cipher( iv, CCM_GCM, crtxt, crtxtLen, pntxt );
    CCMtag( iv, aData, pntxt, aDataLen, crtxtLen, T );
    BURN( RoundKey );

    if (MISMATCH( mac, T, CCM_TAG_LEN ))
    {
...
static void GCM_SIVsetup( const uint8_t* key, const uint8_t* nonce, block_t AK )
{
    ...
    k = h = iv + BLOCKSIZE;
    memcpy( iv + 4, nonce, GCMSIV_NONCE_LEN );
                      ...
                      const void* crtxt, const size_t crtxtLen, void* pntxt )
{
    ...
    if (MISMATCH( P, tag, GCMSIV_TAG_LEN ))
                      ...
                      const void* crtxt, const size_t crtxtLen, void* pntxt )
{
    ...
    if (MISMATCH( auth, tag, EAX_TAG_LEN ))
#endif
 ...
 */
static void OCB_cipher( const uint8_t* key,
                        const uint8_t* nonce, const size_t ptextLen,
                        const void* aData, const size_t aDataLen,
                        const size_t dataSize, void* data, block_t tag )
{
    ...
    count_t i, n = nonce[-1 + OCB_NONCE_LEN] % 64;
                      ...
                      const void* crtxt, const size_t crtxtLen, void* pntxt )
{
    ...
    if (MISMATCH( auth, tag, OCB_TAG_LEN ))
    {
        SABOTAGE( pntxt, crtxtLen );
        return M_AUTHENTICATION_ERROR;
    }
                     ...
                     const void* wrapped, const size_t wrapLen, void* secret )
{
    ...
    block_t A;
    count_t n, i = wrapLen / HB;                 /*  number of semi-blocks    */

    if (i-- < 3 || wrapLen % HB)  return M_DATALENGTH_ERROR;
