...
static void InvMixColumns( state_t *state )
{
    ...
    {
        COPY32BIT( (*state)[i], col[0] );
        (*state)[i][0] = invGM( col[0], col[1], col[2], col[3] );
        (*state)[i][1] = invGM( col[1], col[2], col[3], col[0] );
        (*state)[i][2] = invGM( col[2], col[3], col[0], col[1] );
        (*state)[i][3] = invGM( col[3], col[0], col[1], col[2] );
    }
}
                      ...
                      uint8_t* crtxt, block_t auTag )
{
    ...
    xorBlock( iv, cbc );                         /*  tag = Enc(iv) ^ CBC-MAC  */
    memcpy( auTag, cbc, CCM_TAG_LEN );
}
                      ...
                      uint8_t* crtxt, block_t auTag )
{
    ...
    rijndaelEncrypt( S, auTag );                 /* ..to get auth. tag        */
    CTR_Cipher( auTag, 0, pntxt, ptextLen, crtxt );
    BURN( RoundKey );
}
                      ...
                      const uint8_t tagLen, uint8_t* pntxt )
{
    ...
    GCMSIV_Init( key, nonce, H );                /* get authentication subkey */
    CTR_Cipher( tag, 0, crtxt, crtxtLen, pntxt );
 ...
 * dataSize. psize = dataSize only if input data is plaintext, else it is zero.
 * tag is initialized by plaintext checksum, and then PMAC of aData is added. */
                        ...
                        const size_t dataSize, void* data, block_t tag )
{
    ...
    xMac( data, psize, NULL, &nop, tag );        /*  get plaintext? checksum  */

    for (i = 0; i++ < n; y += BLOCKSIZE)
    {
        getDelta( i, Ld, kt, del );              /*  calculate Δ_i using      */
        xorBlock( del, y );                      /*  .. my 'magic' algorithm  */
        cipher( y, y );
    ...
    if ((i = dataSize % BLOCKSIZE) != 0)         /*  Y_* = Enc(L_* ^ Δ_n) ^ X */
    {                                            /*  and pad X_* or checksum  */
        tag[i] ^= 0x80;
        xorBlock( Ls, del );
        mixThenXor( &rijndaelEncrypt, del, kt, y, i, y );
    }

    xMac( data, dataSize - psize, NULL, &nop, tag );
    cMac( Ld, NULL, del, BLOCKSIZE, tag );       /*  tag = Enc(S ^ Δ_* ^ L_$) */
    n = aDataLen / BLOCKSIZE;

    for (i = 0; i < n; a += BLOCKSIZE)           /*  calculate PMAC of ADATA: */
    {
        getDelta( ++i, Ld, a, del );
        rijndaelEncrypt( del, del );             /*  Δ = Enc( A_i ^ Δ_i )     */
        xorBlock( del, tag );                    /*  add Δ to the tag         */
    }
    if ((n = aDataLen % BLOCKSIZE) != 0)
    {
        memset( kt, 0, BLOCKSIZE );
        getDelta( i, Ld, kt, del );
        cMac( NULL, Ls, a, n, del );             /*  Δ = Enc(L_* ^ A_* ^ Δ_n) */
        xorBlock( del, tag );                    /*  add Δ to the tag         */
    }
}
...
{
    uint8_t result[SP] = { 0 }, n = SP, i;
    while (n)
    {                                            /*  Y = [Y_0][Y_1]...[Y_n]   */
        uint8_t s = --n ? 8 : 0;                 /* multiply X by MSB of Y    */
                   ...
                   const void* data, const size_t dataSize, block_t mac )
{
    ...
    if (poly[0] > 0xFA && q == 12)               /* ..may still be >= 2^130-5 */
                        ...
                        rbase_t* Xa )
{
    ...
    {
        FF1round( i++, P, u, len, C );           /*  encryption rounds        */
        rxbaseAdd( C, u, (i & 1) ? Xa : C - u );
    }                                            /*  B is at (C - u)          */
    for (i ^= 10; i > (0); u = len - u)
    {
        FF1round( --i, P, u, len, C );           /*  decryption rounds        */
        rxbaseSub( C, u, (i & 1) ? C - u : Xa );
    }
}
#else                                            /*         FF3/FF3-1         */
...
static void FF3_Cipher( const uint8_t* key, const char mode,
                        const uint8_t len, const uint8_t* tweak, rbase_t* Xa )
{
    rbase_t* C = Xa + len;
    uint8_t T[8], u = (len + mode) / 2, *k = (void*) C, i;

    memcpy( k, tweak, FF3_TWEAK_LEN );
#if FF3_TWEAK_LEN < 8                            /*  old version of FF3 had a */
    ...
    {
        FF3round( i++, T, u, len, C );           /*  encryption rounds        */
        rxbaseAdd( C, u, (i & 1) ? Xa : C - u );
    }                                            /*  B is at (C - u)          */
    for (i ^= 8; i > (0); u = len - u)
    {
        FF3round( --i, T, u, len, C );           /*  decryption rounds        */
        rxbaseSub( C, u, (i & 1) ? C - u : Xa );
    }
}
#endif /* FF_X */
...
static char FPEinit( const string_t str, const size_t len, rbase_t** indices )
{
    ...
    size_t u = (len + 1) / 2;
    size_t c = (len + u) * sizeof (rbase_t);

#if FF_X != 3                                    /*  extra memory is needed.. */
    bb = (size_t) (LOGRDX * u + 8 - 1e-10) / 8;  /*  to store NUM_radix and.. */
    c += (bb + 3 + BLOCKSIZE) & ~LAST;           /*  mix it in Feistel rounds */
#else
    u *= len > MAXLEN ? 0 : sizeof (rbase_t);
    c += u >= KEYSIZE ? 0 : KEYSIZE - u;
#endif

    if (len < MINLEN || u == 0)  return 'L';     /*  invalid string-length    */
    ...
    *indices = malloc( c );
    ...
    for (u = len; u--; )
    {
        for (c = 0; c != RADIX && alpha[c] != str[u]; ++c);
