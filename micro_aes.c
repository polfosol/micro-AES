...
#define COPY32BIT(y, x)   *(int32_t*) &x =  *(int32_t*) &y
#define XOR4BYTES(y, x)   *(int32_t*) &x ^= *(int32_t*) &y
#define getSBoxValue(x)    ( sbox[x])
#define getSBoxInvert(x)   (rsbox[x])
                      ...
                      uint8_t* crtxt, block_t auTag )
{
    ...
    XOR4BYTES( nonce[0], S[0] );
    XOR4BYTES( nonce[4], S[4] );
    XOR4BYTES( nonce[8], S[8] );                 /* xor POLYVAL with nonce    */
...
#endif
{
    ...
    XOR4BYTES( mac[12], crtxt[ptextLen] );
#else
                      ...
                      uint8_t* pntxt )
{
    ...
    XOR4BYTES( crtxt[crtxtLen], tag[12] );
    XOR4BYTES( mac[12], tag[12] );
    mac[12] &= 0x7F;
    mac[14] &= 0x7F;                             /*  clear 2 bits to get N'   */
...
/*----------------------------------------------------------------------------*\
        OCB-AES (offset codebook mode): how to paralellize the algorithm
                by independent calculation of the offset values
                 + auxiliray functions along with the main API
\*----------------------------------------------------------------------------*/
#if IMPLEMENT(OCB)
...
{
    uint8_t *kt = OCBSubkeys[2], *del = OCBSubkeys[3], *y = data;
                      ...
                      uint8_t* crtxt, block_t auTag )
{
    ...
    memset( OCBSubkeys[1], 0, 2 * BLOCKSIZE );
